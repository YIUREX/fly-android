...
const App: React.FC = () => {
  ...
  // Temporizador de seguridad para evitar que la UI se quede bloqueada si el anuncio no responde
  useEffect(() => {
      let timeout: any;
      if (isWatchingAd) {
          timeout = setTimeout(() => {
              console.warn("Ad safety timeout triggered.");
              setIsWatchingAd(false);
          }, 8000); // 8 segundos de margen para que el anuncio se inicie
      }
  ...
  const handleMissionUpdate = useCallback((type: string, amount: number) => {
      ...
      setStats(prev => {
        ...
        return {
          ...
          totalMissilesDodged: type === 'score' && amount === 25 ? prev.totalMissilesDodged + 1 : prev.totalMissilesDodged, // 25 is Graze score
  ...
  const handleReviveRequest = (method: 'ad' | 'coins') => {
      ...
      if (method === 'coins') {
          ...
          if (window.Android && typeof window.Android.showRewardedAd === 'function') {
              ...
              // Simulación en web si no hay puente android
              console.log("Mocking ad...");
