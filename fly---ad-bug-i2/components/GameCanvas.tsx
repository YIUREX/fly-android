  ...
  const update = useCallback((time: number) => {
    ...
    if (gameState === GameState.PLAYING || gameState === GameState.PAUSED) {
        const drawTrail = (trail: Vector[], style: TrailStyle, lineWidth: number, currentPos?: Vector) => {
            ...
            // Fixed: Removed extra 5th argument to ctx.fillRect
            else if (style.type === 'pixel') { trail.forEach((pos, i) => { if (i % 2 === 0) { const screen = worldToScreen(pos, width, height); ctx.fillRect(screen.x - (lineWidth*1.5)/2, screen.y - (lineWidth*1.5)/2, lineWidth*1.5, lineWidth*1.5); } }); } 
